...
def build_graph(seq_len, input_dim, output_dim, embedding_dim, learning_rate):
    ...
    x = tf.placeholder(tf.float32, [None, seq_len])
    y = tf.placeholder(tf.float32, [None, 1])
    
    w = tf.Variable(tf.ones([seq_len, 1], dtype=tf.float32), dtype=tf.float32)
    b = tf.Variable(tf.ones([input_dim], dtype=tf.float32), dtype=tf.float32)
    
    pred = tf.add(tf.matmul(x, w), b)
    pred_argmax = tf.argmax(tf.nn.softmax(pred), axis=1)
    
    loss = tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits=pred_argmax, labels=y))
    optimizer = tf.train.GradientDescentOptimizer(0.01).minimize(loss)
    
    # adding hidden layers
    last_dim = input_dim
    for dim in [embedding_dim, embedding_dim]:
        hidden = tf.Variable(tf.random_uniform([last_dim, dim], -0.1, 0.1, dtype=tf.float32))
        b = tf.Variable(tf.random_uniform([dim], -0.1, 0.1, dtype=tf.float32))
        x = tf.add(tf.matmul(x, hidden), b)
        x = tf.nn.relu(x)
        last_dim = dim
        
    # defining the output layer
    #out_w = tf.Variable(tf.random_uniform([last_dim, output_dim], -0.1, 0.1, dtype=tf.float32))
    #b = tf.Variable(tf.random_uniform([output_dim], -0.1, 0.1, dtype=tf.float32))
    #out = 
...
def main():
    ...
    input_dim, output_dim, x_train, y_train = prepare_data(
            left_context_len, right_context_len, training_size)
    x, y, optimizer, loss, pred_argmax = build_graph(
            seq_len, input_dim, output_dim, embedding_dim, learning_rate)
