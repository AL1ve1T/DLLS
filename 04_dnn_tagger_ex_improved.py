...
def build_graph(seq_len, input_dim, output_dim, embedding_dim, learning_rate):
    ...
    print(net.get_shape())
    num_layers = 2
    fc_size = embedding_dim * seq_len
    
    # DNN: create 3 hidden layers with a size of 128
    hid_dim = 128
    for i in range(num_layers):
        hid_layer = tf.Variable(tf.random_uniform([fc_size, hid_dim], -0.1, 0.1, dtype=tf.float32))
        hid_b = tf.Variable(tf.random_uniform([hid_dim], -0.1, 0,1))
        net = tf.add(tf.matmul(net, hid_layer), hid_b)
        net = tf.nn.relu(net)
        fc_size = hid_dim
